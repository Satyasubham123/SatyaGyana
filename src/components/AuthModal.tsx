...
export default function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  return (
    /* 🚀 Holy Grail Layout: Outer wrapper scrolls, inner wrapper is forced to be at least screen-height */
    <div className="fixed inset-0 z-[9999] bg-black/90 backdrop-blur-md overflow-y-auto">
      <div className="flex min-h-screen items-center justify-center p-4 sm:p-8">
        
        {/* 🚀 Rigid Block Container: No flex-col, no max-h, no hidden overflow. It CANNOT collapse. */}
        <div className="w-full max-w-md bg-slate-900 rounded-[32px] border border-slate-700 shadow-2xl pb-2">
          ...
          {/* Form Body - Unrestricted height block */}
          <div className="p-8">
            {error && (
            ...
            <form onSubmit={handleEmailAuth} className="space-y-5 block">
              {mode === 'signup' && (
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-4 block">Full Name</label>
                  <div className="relative">
                    <User className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-500" />
                    <input 
                      type="text" required value={name} onChange={e => setName(e.target.value)}
                      className="w-full bg-slate-800 border border-slate-700 p-4 pl-12 rounded-2xl text-white font-bold outline-none focus:border-brand transition-all block"
                    />
                  </div>
                </div>
              )}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-4 block">Email Address</label>
                <div className="relative">
            ...
            <div className="mt-8 pt-6 border-t border-slate-800 block">
              <button 
